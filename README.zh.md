...
cargo run
```
程序会打开标题为“DORA小飞机模拟器”的窗口，左上角显示“控制小飞机”标签。资源文件位于 `assets/images` 与 `assets/fonts`，添加新素材时请保持该目录结构。
...
## 飞行轨迹
飞机行进时会记录路径点，并使用 Bevy gizmos 绘制白色折线。轨迹最多保留 4096 个点，超出后会丢弃最早的点。单击右上角距边缘 20 像素的“清除轨迹”按钮可重置轨迹，按钮在悬停和按下时会通过颜色反馈状态。
